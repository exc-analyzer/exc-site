...
export interface FeedItem {
  ...
  author_accent: AccentId | null;
  author_accent_two: AccentId | null;
  author_shape: AvatarShape | null;
...
export async function loadFeed(
  ...
  if (term) {
    ...
    if (safe) {
      query = query.or(
        `body.ilike.%${safe}%,owner.ilike.%${safe}%,repo.ilike.%${safe}%,author_login.ilike.%${safe}%`,
      );
    }
  }
...
export async function loadRepoFeed(
  owner: string,
  repo: string,
  limit = 25,
): Promise<FeedItem[]> {
...
export type PostStanding = "missing" | "deleted" | "private" | "open";

export async function postStanding(id: string): Promise<PostStanding> {
  if (!supabase) return "missing";
  const { data, error } = await supabase.rpc("post_standing", { post: id });
  if (error) return "missing";
  return (data as PostStanding) ?? "missing";
}
...
export async function deletePost(id: string): Promise<string | null> {
  ...
  const { error } = await supabase
    .from("posts")
    .update({ body: "[removed]", deleted_at: new Date().toISOString() })
    .eq("id", id);
