...
import { ScoreCaveat } from "./Caveat";
...
function AnalysisView({ data }: { data: A }) {
  const daysSinceUpdate = data.updatedAt
    ...
    : null;
  const topTotal = data.topContributors.reduce(
    (sum, c) => sum + c.contributions,
    0,
  );
  const topShare =
    topTotal > 0 ? data.topContributors[0].contributions / topTotal : 0;
        ...
        <div className="space-y-8 p-6">
          <Stats
            items={[
              ...
              {
                label: "Open issues",
                value: data.openIssues.toLocaleString(NUM),
              },
              {
                label: "Pull requests",
                value: data.totalPullRequests.toLocaleString(NUM),
              },
            ]}
          />
            ...
            {data.topContributors.length === 0 ? (
              ...
              <ul className="space-y-2.5">
                {data.topContributors.map((c) => {
                  const share =
                    topTotal > 0 ? (c.contributions / topTotal) * 100 : 0;
                  return (
                    <li key={c.login}>
                      <div className="mb-1 flex justify-between text-sm">
                      ...
                      <Bar
                        percent={share}
                        tone={concentrated && share > 80 ? "warn" : "info"}
                      />
                    </li>
                  );
                })}
            ...
            <KeyValues
              items={[
                ...
                },
                {
                  ...
                  value: <code>{data.defaultBranch}</code>,
                },
                { label: "License", value: data.license ?? "None" },
                {
                  label: "Commits examined",
                  value: String(data.commitsAnalyzed),
                },
                {
                  label: "Recent committers",
                  value:
                    data.topCommitters.map((c) => c.name).join(", ") || "—",
                },
              ]}
            />
          </Details>
...
function SecurityScoreView({ data }: { data: S }) {
  ...
  const tone: Tone =
    data.score >= 90 ? "good" : data.score >= 75 ? "warn" : "bad";
  const headline = `${data.checksPassed} of ${data.evaluatedCount} public checks passed${
    data.unknownCount > 0
      ? ` · ${data.unknownCount} could not be read`
      : ""
  }`;
  ...
  if (failing.length > 0) {
    parts.push(
      `Every one of them is fixable. Start with ${failing[0].label.toLowerCase()}.`,
    );
  }
  if (unknown.length > 0) {
    parts.push(
  ...
  return (
    <div className="space-y-5">
      ...
      />
      <ScoreCaveat scannedAt={data.scannedAt} />
      {actions.length > 0 && (
        <Card>
          <div className="p-6">
                ...
                return (
                  ...
                  >
                    <span className="flex min-w-0 items-start gap-3">
                      <span className={`w-3 shrink-0 ${toneText(t)}`}>
                        {mark}
                      </span>
                    ...
                    <span className="shrink-0 text-right text-xs text-[var(--color-muted)]">
                      {c.detail}
                      {c.status === "fail" && (
                        <span className="ml-2 text-[var(--color-bad)]">
                          −{c.weight}
                        </span>
                      )}
                    </span>
    ...
    missing.length === 0 && weak.length === 0
      ...
      : [
          ...
          missing.length > 0
            ...
            : "",
          weak.length > 0
            ? `Too thin: ${weak.map((m) => m.file).join(", ")}.`
            : "",
        ]
          .filter(Boolean)
          .join(" ");
...
function ContribImpactView({ data }: { data: CI }) {
  const total = data.contributors.reduce(
    (sum, c) => sum + Math.max(0, c.score),
    0,
  );
  const topShare =
    total > 0 && data.contributors.length > 0
      ? Math.max(0, data.contributors[0].score) / total
      : 0;
  const concentrated = topShare > 0.7;

  const tone: Tone =
    data.contributors.length === 0 ? "muted" : concentrated ? "warn" : "good";
  const headline =
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <div className="p-6">
          {data.statsPending ? (
            <Empty>
              GitHub is still computing this repository&apos;s statistics. Try
            ...
            <ul className="space-y-4">
              {data.contributors.map((c) => {
                const share =
                  total > 0 ? (Math.max(0, c.score) / total) * 100 : 0;
                return (
                  <li key={c.login} className="flex items-center gap-4">
                    <img
                      src={`https://avatars.githubusercontent.com/${c.login}?s=64`}
                      alt=""
                      width={32}
                      height={32}
                      loading="lazy"
                      referrerPolicy="no-referrer"
                      className="size-8 rounded-full bg-[var(--color-raised)]"
                    />
  ...
  const summary =
    commits.length === 0
      ...
      : [
          ...
          first && first !== last
            ? `The oldest in this window is from ${formatDate(first.date)}.`
            : "",
          single
...
function ActionsAuditView({ data }: { data: AA }) {
  ...
  const serious = findings.filter(
    (f) => f.severity === "critical" || f.severity === "risky",
  );
  const warnings = findings.filter((f) => f.severity === "warning");
  const unreadable = data.workflows.filter((wf) => wf.severity === "error");

  const tone: Tone =
    serious.length > 0 ? "bad" : warnings.length > 0 ? "warn" : "good";
  const headline =
    serious.length > 0
  ...
  const clean = data.workflows
    .filter((wf) => wf.severity === "ok")
    .map((wf) => wf.name);
              ...
              {ordered.map((wf) => (
                <div key={wf.path}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <Badge tone={WF_TONE[wf.severity]}>
                      {WF_LABEL[wf.severity]}
                    </Badge>
                  </div>
                  ...
                  <ul className="mt-2 space-y-2">
                    {wf.findings.map((f, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <span
                          className={`mt-0.5 shrink-0 text-xs ${toneText(WF_TONE[f.severity])}`}
                        >
                          {f.severity === "ok" ? "✓" : "!"}
                        </span>
                        <div>
...
function UserAnalysisView({ data }: { data: UA }) {
  ...
  const summary = [
    ...
    data.languages.length > 0
      ? `Mostly works in ${data.languages[0].name}.`
      : "",
  ]
    .filter(Boolean)
    .join(" ");
        ...
        <div className="space-y-8 px-6 py-5">
          <Stats
            items={[
              ...
              {
                label: "Repositories",
                value: data.publicRepos.toLocaleString(NUM),
              },
...
function ScanSecretsView({ data }: { data: SS }) {
  ...
  return (
    <div className="space-y-5">
      <SensitiveNotice>
        This result is not saved and has no shareable address. Values are shown
        masked and the raw key is never stored anywhere. If you find something
        in someone else&apos;s repository, tell the owner rather than publishing
        it.
      </SensitiveNotice>
