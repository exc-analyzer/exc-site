    ...
    <section className="rounded-xl border border-[var(--color-line)] bg-[var(--color-surface)] p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--color-muted)]">
            If a command reports that it could not reach something, this tells you which host is
            being blocked.
          </p>
        </div>
        <button
            ...
            return (
              <li key={probe.name} className="flex items-start justify-between gap-4 py-2.5">
                ...
                <div className="shrink-0 text-right text-xs">
                  ...
                  {r.kind === 'ok' && <span className="text-emerald-400">Reached · {r.ms} ms</span>}
                  {r.kind === 'http' && (
                  ...
                  {r.kind === 'skipped' && (
                    <span className="text-[var(--color-muted)]">Needs a token</span>
                  )}
                </div>
              </li>
            );
      ...
      {anyBlocked && (
        <p className="mt-4 rounded-lg border border-red-900/60 bg-red-950/25 px-4 py-3 text-xs text-red-200/90">
          At least one host is unreachable. The usual causes are an ad or tracker blocker, a browser
          shield, a corporate network filter, an ISP block, or a VPN. Try opening the same address
          directly in a tab: if that fails too, the block is outside this page.
        </p>
      )}
    </section>
  );
